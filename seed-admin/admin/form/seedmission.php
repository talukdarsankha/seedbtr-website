...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["aboutImage"]["name"])) {
        ...
        $count= $crud ->Count("seedmission","1");
       
        if($count<1){
            $create = $crud->Create($data,"seedmission");

            if($create) {
                $_SESSION['message']= '<script>alert("Data Uploaded!");</script>';
                echo '<script>window.location.assign("../view_seedmission.php");</script>';
            }
        } else {
            
            $create =$crud->Create($data,"seedvision");
            ...
            if (move_uploaded_file($_FILES["aboutImage"]["tmp_name"], "../".$target_file)) {
              ...
              // $_SESSION['message']= "The file ". basename( $_FILES["aboutImage"]["name"]). " has been uploaded.";
                ...
                if($count<1){
                    ...
                    $update =$crud->Create($data,"seedvision");
