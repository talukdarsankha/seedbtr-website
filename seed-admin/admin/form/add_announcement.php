...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["pdf_file"]["name"])) {
        ...
        $count= $crud ->Count("announcements","1");
       
        if($count<1){
            $create = $crud->Create($data,"announcements");

            if($create) {
                $_SESSION['message']= '<script>alert("Data Uploaded!");</script>';
                echo '<script>window.location.assign("../view_announcement.php");</script>';
            }
        } else {
            ...
            if (move_uploaded_file($_FILES["pdf_file"]["tmp_name"], "../".$target_file)) {
              ...
              // $_SESSION['message']= "The file ". basename( $_FILES["aboutImage"]["name"]). " has been uploaded.";
                ...
                if($count<1){
                    ...
                    $update =$crud->Create($data,"announcements");
