...
if(isset($_POST['submit'])){
    ...
    if (empty($_FILES["pdf_file"]["name"])) {
        ...
        $count= $crud ->Count("newsticker","1");
       
        if($count<1){
            $create = $crud->Create($data,"newsticker");
              ...
              // $_SESSION['message']= "The file ". basename( $_FILES["pdf_file"]["name"]). " has been uploaded.";
                ...
                if($count<1){
                    ...
                    if($create) {
                        ...
                        echo '<script>window.location.assign("../newsticker.php");</script>';
                    }
                } else {
                    
                    $update =$crud->Create($data,"newsticker");
