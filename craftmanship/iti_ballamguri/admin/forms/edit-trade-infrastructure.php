...
if(isset($_POST['id'])){
    ...
    if (empty($_FILES["image"]["name"])) {
        $data = [
            'Unit' =>$unit,
            'ClassRoomArea' =>$classroom_area_per_unit,
            'TotalClassRoom' =>$total_classroom_area,
            'WorkshopArea' =>$workshop_area_per_unit,
            'Total_WorkshopArea' =>$total_workshop_area,
            'Trade' =>$trade,         
            'date_uploaded' =>$date          
        ];
        $count= $crud ->Count("trade_infrastructure","`id`='$id'");
        if($count==1){
            $update =$crud->Update("trade_infrastructure",$data,"`id`='$id'");
            $data["successMessage"]="data updated successfully.";
        } else {
            $data["errorMessage"]="Error updating data.";
        }
    } else {
        $target_dir = "images/tradeInfrastructure/";
