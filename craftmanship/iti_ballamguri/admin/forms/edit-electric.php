...
if(isset($_POST['id'])){
    ...
    if (empty($_FILES["image"]["name"])) {
        $data = [
            'electric_load' =>$present_electric_load,
            'Date_of_Connection' =>$connection_date,
            'Connection_name' =>$connection_name,       
        ];
        $count= $crud ->Count("electric_power_supply","`id`='$id'");
        if($count==1){
            $update =$crud->Update("electric_power_supply",$data,"`id`='$id'");
            $data["successMessage"]="data updated successfully.";
        } else {
            $data["errorMessage"]="Error updating data.";
        }
    } else {
        $target_dir = "images/electricPowerSupply/";
