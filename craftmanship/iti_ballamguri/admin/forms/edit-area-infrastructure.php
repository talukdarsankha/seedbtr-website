...
if(isset($_POST['id'])){
    ...
    if (empty($_FILES["image"]["name"])) {
        $data = [
            ...
            'TotalClassRoomsArea' =>$total_classroom_area,
            'TotalWorkshopsArea' =>$total_workshop_area,
            'DrawingHall' =>$drawing_hall,
            'AudioVisualHall' =>$visual_audio_hall,
            'ComputerLab' =>$computer_lab,
            'Library' =>$library,
            'date_uploaded' =>$date          
        ];
        $count= $crud ->Count("area_infrastructure","`id`='$id'");
        if($count==1){
            $update =$crud->Update("area_infrastructure",$data,"`id`='$id'");
            $data["successMessage"]="data updated successfully.";
        } else {
            $data["errorMessage"]="Error updating data.";
        }
    } else {
        $target_dir = "images/areaInfrastructure/";
